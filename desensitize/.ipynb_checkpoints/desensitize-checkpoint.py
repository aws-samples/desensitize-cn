...
class Desensitize(object):
    """The Desensitize class is used to clean personal information out of dirty
    dirty text. It manages a set of ``Detector``'s that are each responsible
    for identifying their particular kind of ``Implement``.
    """
    ...
    def clean(self, text, **kwargs):
        """This is the master method that cleans all of the implement out of the
        dirty dirty ``text``. All keyword arguments to this function are passed
        through to the  ``Implement.replace_with`` method to fine-tune how the
        ``Implement`` is cleaned.
        """
    ...
    def iter_filth(self, text):
        ...
        # inline instead of with a Implement.__cmp__ method, which is apparently
